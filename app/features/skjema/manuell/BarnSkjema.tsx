...
type Props = {
  barnIndex: number;
  vedFjernBarn: () => void | undefined;
};

export const BarnSkjema = ({ barnIndex, vedFjernBarn }: Props) => {
  const { t } = useOversettelse();
  const form = useFormContext<ManueltSkjema>();

  const barnField = useFormScope(form.scope(`barn[${barnIndex}]`));

  const barnFieldValues = barnField.value();
  ...
  const borHosEnForelder = barnFieldValues.bosted === "IKKE_DELT_FAST_BOSTED";
